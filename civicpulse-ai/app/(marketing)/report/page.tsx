...
const COT_STEPS = [
  ...
  "Routing to Municipal Public Works..."
];
...
export default function ReportPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCotIndex((prev) => Math.min(prev + 1, COT_STEPS.length - 1));
    }, 600);
        ...
        try {
          const res = await fetch(`${apiUrl}/api/triage`, {
            ...
            body: JSON.stringify({
              ...
              description: "User uploaded image via dashboard"
            }),
          });
          ...
          setIsScanning(false);
          setError(fetchErr.message || "Failed to communicate with AI Engine.");
        }
      };
      reader.onerror = () => {
        setIsScanning(false);
    ...
    <div className="pt-32 pb-20 px-4 max-w-4xl mx-auto min-h-screen flex flex-col">
      <div className="text-center mb-12">
        ...
        <p className="text-slate-400">
          Upload a photo. Gemma 4 handles the rest.
        </p>
      </div>

      <GlassCard className="flex-1 flex flex-col items-center justify-center min-h-[400px]">
        {error && (
        ...
        {!isScanning && !result && (
          ...
          >
            ...
            <h3 className="text-xl font-heading font-bold text-white mb-2">Upload Photo/Video</h3>
            <p className="text-slate-400 text-sm mb-6">Drag and drop or click to select</p>
            <Button variant="secondary" onClick={(e) => { e.stopPropagation(); triggerFileInput(); }}>Select File</Button>
          </div>
        )}
          ...
          <div className="w-full max-w-lg text-center flex flex-col items-center">
            <div className="relative w-64 h-64 bg-white/5 rounded-xl border border-white/10 overflow-hidden mb-8 mx-auto flex items-center justify-center">
              ...
              <div className="absolute inset-0 z-10 pointer-events-none">
                <div className="w-full h-1 bg-cyan-400 shadow-[0_0_15px_#06b6d4] animate-scan" style={{ top: "0%", position: "absolute" }}></div>
              </div>
            </div>
            <h3 className="text-2xl font-heading font-bold text-white mb-2 animate-pulse">Analyzing with Gemma 4...</h3>
            <div className="h-6 overflow-hidden relative w-full flex justify-center">
              <AnimatePresence mode="wait">
                ...
                >
                  {COT_STEPS[cotIndex]?.toUpperCase() || "PROCESSING..."}
                </motion.p>
              </AnimatePresence>
            </div>
          </div>
        ...
        {result && (
          ...
          >
            ...
            <div className="mt-10 text-center">
              <Button onClick={() => setResult(null)} variant="outline">Report Another Issue</Button>
            </div>
          </motion.div>
        )}
      </GlassCard>
